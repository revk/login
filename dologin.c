...
const char *
forcelogin (SQL * sqlp, const char *session, const char *username, SQL_RES * res)
{
   if (!sqlp)
      return "No sql";
          ...
          (!(r =
             sql_safe_query_store_free (sqlp,
                                        sql_printf ("SELECT * FROM `%#S` WHERE `%#S`=%#s", CONFIG_DB_USER_TABLE,
                                                    CONFIG_DB_USERNAME_FIELD, username))) || !sql_fetch_row (r)))
         r = NULL;
      if (!r)
...
const char *
dologin (SQL * sqlp, const char *session, const char *username, const char *password, const char *otp)
{
   if (!sqlp)
      return "No sql";
   if (otp)
   ...
   SQL_RES *res =
      sql_safe_query_store_free (sqlp,
                                 sql_printf ("SELECT * FROM `%#S` WHERE `%#S`=%#s", CONFIG_DB_USER_TABLE, CONFIG_DB_USERNAME_FIELD,
                                             username));
   ...
   {                            // Login OK but hash needs updating
      ...
      sql_safe_query_free (sqlp,
                           sql_printf ("UPDATE `%#S` SET `%#S`=%#s WHERE `%#S`=%#s", CONFIG_DB_USER_TABLE, CONFIG_DB_PASSWORD_FIELD,
                                       newhash, CONFIG_DB_USERNAME_FIELD, username));
#endif
      free (newhash);
   }
