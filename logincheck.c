...
SQL_RES *
find_session (SQL * sqlp, const char *session, int envstore)
{
   SQL_RES *found = NULL;
#ifdef CONFIG_DB_SEPARATE_SESSION
   SQL_RES *res =
      sql_safe_query_store_free (sqlp,
                                 sql_printf ("SELECT * FROM `%#S` WHERE `%#S`=%#s", CONFIG_DB_SESSION_TABLE,
   ...
   if (sql_fetch_row (res))
   {
      ...
      if (*CONFIG_DB_SESSION_AGENT && (a = getenv ("HTTP_USER_AGENT")) && (b = sql_colz (res, CONFIG_DB_SESSION_AGENT))
          && strcmp (a, b))
   ...
   SQL_RES *res =
      sql_safe_query_store_free (sqlp,
                                 sql_printf ("SELECT * FROM `%#S` WHERE `%#S`=%#s", CONFIG_DB_USER_TABLE, CONFIG_DB_SESSION_FIELD,
                                             session));
   if (sql_fetch_row (res))
      found = res;
#endif
               ...
               *pass++ = 0;
               ...
               SQL_RES *res =
                  sql_safe_query_store_free (&sql,
                                             sql_printf ("SELECT * FROM `%#S` WHERE `%#S`=%#s", CONFIG_DB_USER_TABLE,
                                                         CONFIG_DB_USERNAME_FIELD, auth));
               if (!sql_fetch_row (res))
                  fail = "Login failed.";
               else
               {
                  ...
                  {             // Login OK but hash needs updating
                     ...
                     sql_safe_query_free (&sql,
                                          sql_printf ("UPDATE `%#S` SET `%#S`=%#s WHERE `%#S`=%#s", CONFIG_DB_USER_TABLE,
                                                      CONFIG_DB_PASSWORD_FIELD, newhash, CONFIG_DB_USERNAME_FIELD, auth));
#endif
                     free (newhash);
                  }
                  if (newhash)
