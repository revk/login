...
static char *password_check_sha256(const char *hashtext, const char *password)
...
static char *password_check_sha1(const char *hashtext, const char *password)
...
static char *password_check_md5(const char *hashtext, const char *password)
...
#ifdef	CONFIG_PASSWORD_MD5
static char *password_check_md5p(const char *hashtext, const char *password)
...
static char *password_check_mysql_password(const char *hashtext, const char *password)
{
   const char *hash = hashtext;
   if (*hash++ != '*')
      return NULL;              // Starts *
...
static char *password_check_mysql_old_password(const char *hashtext, const char *password)
{
   // Check it looks right
   size_t l = 0;
...
static char *password_check_argon2i(const char *hashtext, const char *password)
{
   int e;
...
static char *password_check_argon2d(const char *hashtext, const char *password)
{
   int e;
...
{                               // Check if looks like a hash
   if (!hash || !*hash)
      return 0;                 // Nope
   size_t l = strlen(hash);
   int ishex(const char *p, int l) {    // is it hex?
   ...
   if (l >= MD5_DIGEST_LENGTH * 2 + 4 && !strncmp(hash, "MD5#", 4) && ishex(hash + 4, MD5_DIGEST_LENGTH * 2))
      return 5;
   if (l >= MD5_DIGEST_LENGTH * 2 + 5 && !strncmp(hash, "MD5P#", 5) && ishex(hash + l - MD5_DIGEST_LENGTH * 2, MD5_DIGEST_LENGTH * 2))
      return 5;
